            ...
            try:
                ...
                async with httpx.AsyncClient(headers=http_task_headers,timeout=30.0, follow_redirects=True) as client:
                    async for record in result_pubs:
                        pub_node = record["p"]
                        pub_uuid = record.get("uuid")
                        if not pub_uuid:
                            continue
                        results["processed"] += 1

                        original_url = pub_node.get("identifier#url")
                        self.logger.debug(f"Processing Publication node {pub_uuid} with URL: {original_url}")
                        ...
                        try:
                            ...
                            if final_url != original_url:
                                ...
                                # Update the node with the final URL via NodeService
                                await service.set_node_properties(UUID(pub_uuid), {"identifier#url": final_url})
                                self.logger.info(f"Updated node {pub_uuid} URL from {original_url} to {final_url}")
                            
                            self.logger.debug(f"Successfully fetched homepage for {final_url}.")
                            
                        except (httpx.HTTPStatusError, httpx.RequestError) as e:
                            self.logger.warning(f"URL {original_url} for node {pub_uuid} is not active: {e}")
                            results["url_not_active"] += 1
                            continue
                        except Exception as e:
                            self.logger.error(f"Unexpected error fetching URL {original_url} for node {pub_uuid}: {e}")
                            results["errors"] += 1
                            continue
                        ...
                        if generator_element:
                            ...
                            if "open journal systems" in generator_content or "ojs" in generator_content:
                                self.logger.info(f"Identified OJS system for {current_url} (node {pub_uuid}).")
                                # Update node attribute via NodeService
                                await service.set_node_properties(UUID(pub_uuid), {"sourceSystem": "OJS"})
                                results["ojs_found"] += 1
                                ...
                                if oai_url_found:
                                    # Update node attribute via NodeService
                                    await service.set_node_properties(UUID(pub_uuid), {"identifier#oaiurl": oai_url_found})
                                    results['oai_endpoint_found_list'].append(oai_url_found)
                                    results["oai_endpoint_found"] += 1
                                else:
                                    self.logger.warning(f"No active OAI-PMH endpoint found for OJS site {current_url} (node {pub_uuid}).")
                                    results["oai_endpoint_not_active"] += 1
                            else:
                                self.logger.debug(f"Generator {generator_content} is not OJS for {current_url} (node {pub_uuid}).")
                                results["not_ojs"] += 1
                        else:
                            self.logger.debug(f"No generator meta tag found for {current_url} (node {pub_uuid}).")
